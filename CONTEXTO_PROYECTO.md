...
- **Pruebas: Vitest.** Unit sin BD (`pnpm test`) e integración con `supertest` + Postgres desechable (`pnpm test:integration`, `test:all` corre ambas). **244 casos** de integración en **15 archivos** que cubren **todos los controllers por HTTP** y el job de cierre del día. Para poder montar la app en tests, `app.js` exporta la app y `index.js` solo hace `listen` + jobs. Arnés en `test/` (bootstrap de la BD, seeds, tokens). Las funciones puras de precios/secado/folio viven en `utils/calculosNotas.js`. **Todo en verde** (39 unit + 244 de integración, verificado el 2026-09-04). Las unit incluyen el driver de eWeLink, el trámite OAuth y la regla del reconciliador, con `fetch` y la base mockeados.
