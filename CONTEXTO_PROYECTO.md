...
**Estado de uso:** la infraestructura está desplegada en producción (Netlify + Fly.io + Supabase, desde 2026-06-22) y el sistema **todavía no está en uso real** en la lavandería. El backend corre hoy en la **v32**.

**El control por Sonoff ya se probó contra hardware real (2026-09-03) y funciona de punta a punta**: la máquina **L9 encendió y se apagó por orden de la app**. Ese estreno costó dos correcciones y un cambio de fondo: (1) mandábamos un `Content-Type` que eWeLink rechaza (error 400 antes de mirar credenciales); (2) el plan gratuito **no permite entrar con correo y contraseña** (error 407) y hubo que **reimplementar la autenticación con OAuth 2.0** (mig. 098). Lo que falta es capturar el resto de los Device IDs y que los equipos respondan: los 5 Sonoff enlazados contestan `4002` cuando el local está sin corriente. Como aún no hay datos reales, se pueden correr migraciones y renombres de raíz con libertad.
...
### Control de máquinas (Sonoff) — Fase 1 PROBADA contra hardware real
- `services/dispositivos/` es la **capa de driver**: interfaz `encender/apagar/estado` y selección por `DISPOSITIVOS_DRIVER`. `nullDriver` (simulación en memoria, por defecto en local) y `ewelinkDriver` (nube eWeLink, API v2, sin dependencias: `fetch` + `crypto` nativos). **En producción corre `ewelink`; en local sigue el `null`.**
...
- **Migraciones caseras**: archivos SQL numerados en `db/migrations/`, runner idempotente (`db/migrate.js`) que registra lo aplicado en `schema_migrations`. Bootstrap = `schema.sql` + migraciones en orden. Van por la **101** (95 archivos) y **todas están aplicadas en producción** (release **v32**, 2026-09-03, verificado en los logs del `release_command`). Las últimas: **098** sesión OAuth de eWeLink, **099** cierre automático de caja, **100** eliminación del empaquetado (borra columnas) y **101** cobros atados a su caja + cifras del corte congeladas. La tabla base histórica se llamaba `ordenes` y fue renombrada a `notas` (mig. 009); `schema.sql` conserva el nombre viejo solo como bootstrap.
...
- **DB → Supabase**.
- **Secretos en Fly**: `DATABASE_URL`, `JWT_SECRET`, `EWELINK_APP_ID`, `EWELINK_APP_SECRET`, `EWELINK_REGION` (`us`) y `DISPOSITIVOS_DRIVER=ewelink`. **`EWELINK_EMAIL`, `EWELINK_PASSWORD` y `EWELINK_COUNTRY_CODE` siguen cargados pero YA NO SE USAN** desde que la autenticación pasó a OAuth (mig. 098): conviene borrarlos con `fly secrets unset` para no guardar la contraseña del cliente sin motivo.
...
- **Sonoff, puesta en marcha** — la cadena completa funciona (L9 encendió y apagó). Quedan **5 Sonoff enlazados respondiendo `4002`** ("fallo de control del dispositivo"), lo más probable por estar sin corriente con el local cerrado. Si con la luz puesta siguen así, hay que mirar la **cobertura WiFi** en cada punto.
...
- **RIESGO CERRADO — Sonoff probado contra hardware real** (2026-09-03): L9 encendió y apagó por orden de la app. Lo que queda es de instalación, no de código: los 5 Sonoff enlazados responden `4002` con el local sin corriente, y hay que confirmar la **cobertura WiFi** de cada punto con la luz puesta.
...
1. **Terminar el enlace de los Sonoff**: con el local con corriente, confirmar que los 5 dispositivos dejan de responder `4002` y darles **"Probar enlace"**. Si alguno sigue caído, revisar su **WiFi** antes que el código.
2. **Poner en OFF el "Power-on state"** de cada Sonoff en la app eWeLink, para que un apagón no arranque máquinas solas.
3. **Borrar los secretos muertos** en Fly: `fly secrets unset EWELINK_EMAIL EWELINK_PASSWORD EWELINK_COUNTRY_CODE` (la autenticación ya es OAuth).
4. **Cerrar la decisión del edredón** con el cliente (ver §5) y, según la respuesta, ajustar cómo se cobra esa carga.
5. **Entregar el sistema al cliente** para que empiece el uso real. Antes de entregar: **subir la versión** en `frontend/package.json` (hoy 1.0.0) y **cambiar la contraseña de los usuarios de prueba**.
...
**Trabajo más reciente (2026-09-03, noche):** tres ajustes chicos, sin migraciones.
