...
- **Pruebas: Vitest 4 + Testing Library** (`environment: jsdom`, `setupFiles: src/test/setup.js`), `pnpm test` / `test:watch`. **65 casos en 16 archivos, todos en verde**: helpers puros de `lib/`, la página **Login**, y componentes con lógica (KpiCard, SalesCard, MachineCard, CircularTimer, EmpleadoDeleteModal, EmpleadoEditModal, CashCutCard, SucursalSelector). `api`/`useAuth`/`useNavigate` se mockean con `vi.mock`/`vi.hoisted`.
...
- **Pruebas: Vitest.** Unit sin BD (`pnpm test`) e integración con `supertest` + Postgres desechable (`pnpm test:integration`, `test:all` corre ambas). **229 casos** de integración en **16 archivos** que cubren **todos los controllers por HTTP** y el job de cierre del día. Para poder montar la app en tests, `app.js` exporta la app y `index.js` solo hace `listen` + jobs. Arnés en `test/` (bootstrap de la BD, seeds, tokens). Las funciones puras de precios/secado/folio viven en `utils/calculosNotas.js`. **Todo en verde** (24 unit + 229 de integración, verificado el 2026-09-02).
...
- **Migraciones caseras**: archivos SQL numerados en `db/migrations/`, runner idempotente (`db/migrate.js`) que registra lo aplicado en `schema_migrations`. Bootstrap = `schema.sql` + migraciones en orden. Van por la **097** (91 archivos). Las **091-095** entraron en producción con el release **v27** (2026-09-02, verificado contra Supabase); la **096** (tope congelado por carga) y la **097** (marca de máquina iniciada) **están solo en local** y se aplicarán solas en el próximo `fly deploy`. La tabla base histórica se llamaba `ordenes` y fue renombrada a `notas` (mig. 009); `schema.sql` conserva el nombre viejo solo como bootstrap.
...
- **Topes de precio por carga** (mig. 050/052): limitan `lavadora + secadora + productos`; el **ajuste manual no cuenta** y es **tope duro para todos los roles**, incluido admin. Se valida en backend antes del COMMIT.
...
**Trabajo más reciente (2026-09-02, tarde):** cambio de regla sobre las **máquinas** y una tanda de arreglos que salieron de revisarlo.
