...
- **Pruebas: Vitest 4 + Testing Library** (`environment: jsdom`, `setupFiles: src/test/setup.js`), `pnpm test` / `test:watch`. **65 casos en 16 archivos, todos en verde** (2026-09-03): helpers puros de `lib/`, la página **Login**, y componentes con lógica (KpiCard, SalesCard, MachineCard, CircularTimer, EmpleadoDeleteModal, EmpleadoEditModal, CashCutCard, SucursalSelector). `api`/`useAuth`/`useNavigate` se mockean con `vi.mock`/`vi.hoisted`.
...
- **Apagado de emergencia**: `POST /maquinas/:id/apagar-sonoff` (botón rojo "Apagar ahora"), permitido **incluso con la máquina en uso** —es el botón al que se corre cuando algo quedó andando— y con 3 reintentos, igual que el apagado tras la prueba física.
...
- **Pruebas de frontend incompletas**: hay helpers, componentes con lógica y la página **Login** (65 casos, todos en verde). **Faltan las páginas restantes**, la prioritaria es **`NuevaNota`** (formulario complejo de cargas/productos con tarifas y topes); luego `Empleados`, `Ventas`, `Notas`, `Caja`, `Inventario`. El backend sí está cubierto por completo (**39 unit + 244 de integración en 15 archivos, todas en verde el 2026-09-03**), incluidas las regresiones del día: cortes por día del negocio, folio sellado con el día del negocio, tope congelado por carga, cobro que se desmarca al cambiar el costo, la carrera por una máquina y el ciclo de otra nota que no se debe cortar. Lo único sin prueba propia es el entorno de pruebas (sucursal oculta), verificado a mano contra la API en local y en producción.

---

## 6. Próximo paso inmediato

1. **Arreglar que el reconciliador apague lo encendido a mano** antes de volver a poner `DISPOSITIVOS_DRIVER=ewelink` en producción (ver §1/§2/§5) — si no, cualquier lavadora encendida fuera de la app se vuelve a apagar sola en máximo 3 minutos.
2. **Terminar el enlace de los Sonoff**: con el local con corriente, confirmar que los 5 dispositivos dejan de responder `4002` y darles **"Probar enlace"**. Si alguno sigue caído, revisar su **WiFi** antes que el código. (Requiere reactivar el driver real primero, o probar puntualmente esa máquina.)
3. **Poner en OFF el "Power-on state"** de cada Sonoff en la app eWeLink, para que un apagón no arranque máquinas solas.
4. **Borrar los secretos muertos** en Fly: `fly secrets unset EWELINK_EMAIL EWELINK_PASSWORD EWELINK_COUNTRY_CODE` (la autenticación ya es OAuth).
5. **Cerrar la decisión del edredón** con el cliente (ver §5) y, según la respuesta, ajustar cómo se cobra esa carga.
6. **Entregar el sistema al cliente** para que empiece el uso real. Antes de entregar: **subir la versión** en `frontend/package.json` (hoy 1.0.0) y **cambiar la contraseña de los usuarios de prueba**.
...
**Trabajo más reciente (2026-09-03, tarde-noche):** incidente de producción — Sonoff apagaba lavadoras solas. Solo config, sin cambio de código (release v33).
